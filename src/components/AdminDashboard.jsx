...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .adm-layout{display:flex;min-height:100vh;background:${C.negro};color:${C.crema};font-family:'Lora',serif}

  /* ── SIDEBAR ── */
  .adm-sidebar{
    width:252px;flex-shrink:0;
    background:${C.negro2};
    border-right:0.5px solid ${C.borde};
    display:flex;flex-direction:column;
    position:sticky;top:0;height:100vh;overflow-y:auto;
  }
  .adm-sb-logo{padding:22px 20px 18px;border-bottom:0.5px solid ${C.borde}}
  .adm-sb-logo-name{font-family:'Cormorant Garamond',serif;font-size:20px;font-weight:300;letter-spacing:5px;text-transform:uppercase;color:${C.oro}}
  .adm-sb-logo-sub{font-size:9px;letter-spacing:3px;text-transform:uppercase;color:${C.oroDim};margin-top:3px}

  .adm-sb-section{padding:16px 16px 6px;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:${C.oroDim}}
  .adm-sb-divider{height:0.5px;background:${C.borde};margin:8px 16px}
  ...
  .adm-sb-item:hover{background:${C.verde}}
  .adm-sb-item.active{background:${C.verde2};border-left-color:${C.oro}}
  .adm-sb-item.active .adm-si-label{color:${C.crema}}
  .adm-sb-item.active .adm-si-icon{color:${C.oro}}

  .adm-si-icon{font-size:15px;color:${C.muted};width:18px;flex-shrink:0}
  .adm-si-label{font-size:12px;color:${C.muted};flex:1}
  .adm-si-badge{
    font-size:9px;font-weight:500;min-width:20px;text-align:center;
    padding:2px 7px;border-radius:10px;
    background:${C.verde3};color:${C.oro};
  }
  .adm-si-badge.highlight{background:${C.oro};color:${C.negro}}
  .adm-si-badge.zero{background:transparent;border:0.5px solid ${C.borde};color:${C.oroDim}}

  .adm-sb-footer{margin-top:auto;padding:14px;border-top:0.5px solid ${C.borde}}
  .adm-sb-user{display:flex;align-items:center;gap:10px;padding:10px 12px;background:${C.verde};border-radius:3px;margin-bottom:8px}
  .adm-sb-avatar{width:28px;height:28px;border-radius:50%;background:${C.oroDim};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;color:${C.negro};flex-shrink:0;font-family:'Cormorant Garamond',serif}
  .adm-sb-uname{font-size:12px;color:${C.crema}}
  .adm-sb-urole{font-size:10px;color:${C.muted}}
  .adm-sb-logout{width:100%;padding:9px;background:transparent;border:0.5px solid ${C.borde};color:${C.muted};font-size:10px;letter-spacing:2px;text-transform:uppercase;cursor:pointer;font-family:'Cormorant Garamond',serif;transition:border-color 0.15s,color 0.15s}
  .adm-sb-logout:hover{border-color:${C.danger};color:${C.danger}}

  /* ── MAIN ── */
  .adm-main{flex:1;display:flex;flex-direction:column;min-width:0}
  .adm-topbar{background:${C.negro2};border-bottom:0.5px solid ${C.borde};padding:0 28px;height:54px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
  .adm-topbar-title{font-family:'Cormorant Garamond',serif;font-size:20px;font-weight:300;color:${C.crema}}
  .adm-topbar-title em{font-style:italic;color:${C.oro}}
  .adm-topbar-actions{display:flex;gap:8px}

  .adm-btn{padding:7px 16px;background:transparent;border:0.5px solid ${C.borde};color:${C.muted};font-size:10px;letter-spacing:1.5px;text-transform:uppercase;cursor:pointer;font-family:'Cormorant Garamond',serif;display:flex;align-items:center;gap:6px;transition:border-color 0.15s,color 0.15s}
  .adm-btn:hover{border-color:${C.oroDim};color:${C.oro}}
  .adm-btn.primary{background:${C.oro};color:${C.negro};border-color:${C.oro}}
  .adm-btn.primary:hover{background:${C.oro2}}
  ...
  .adm-kpi{background:${C.negro2};border:0.5px solid ${C.borde};padding:18px 20px}
  .adm-kpi-label{font-size:9px;letter-spacing:3px;text-transform:uppercase;color:${C.oroDim};margin-bottom:8px;font-family:'Cormorant Garamond',serif}
  .adm-kpi-value{font-family:'Cormorant Garamond',serif;font-size:28px;font-weight:300;color:${C.oro};line-height:1}
  .adm-kpi-sub{font-size:11px;color:${C.muted};margin-top:5px}

  /* Section header */
  .adm-sec-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px}
  .adm-sec-title{font-family:'Cormorant Garamond',serif;font-size:18px;font-weight:300;color:${C.crema}}
  .adm-sec-count{font-size:12px;color:${C.muted};margin-left:8px}

  /* Table */
  .adm-table-wrap{background:${C.negro2};border:0.5px solid ${C.borde};overflow:hidden}
  .adm-table-wrap table{border-collapse:collapse;width:100%}
  .adm-table-wrap th{font-family:'Cormorant Garamond',serif;font-size:9px;letter-spacing:2.5px;text-transform:uppercase;color:${C.oroDim};padding:11px 14px;text-align:left;border-bottom:0.5px solid ${C.borde};font-weight:400;white-space:nowrap}
  .adm-table-wrap td{font-family:'Lora',serif;font-size:12px;color:${C.muted};padding:11px 14px;border-bottom:0.5px solid ${C.borde};vertical-align:top}
  .adm-table-wrap tr:last-child td{border-bottom:none}
  .adm-table-wrap tr.clickable:hover td{background:${C.verde};cursor:pointer}
  .td-name{color:${C.crema} !important;font-family:'Cormorant Garamond',serif !important;font-size:14px !important}
  .td-price{color:${C.oro} !important;font-family:'Cormorant Garamond',serif !important;font-size:16px !important;font-weight:300 !important}
  .td-empty{text-align:center;padding:40px 14px !important;color:rgba(245,240,232,0.2) !important;font-style:italic}

  /* Info box */
  .adm-info-box{background:${C.verde};border:0.5px solid ${C.borde};border-left:2px solid ${C.oroDim};padding:14px 18px;margin-bottom:20px;font-size:12px;color:${C.muted};line-height:1.7}
  .adm-info-box strong{color:${C.oro};font-weight:400}

  /* Chart container */
  .adm-chart-wrap{background:${C.negro2};border:0.5px solid ${C.borde};padding:22px 24px;margin-bottom:20px}

  /* Divider */
  .adm-divider{height:0.5px;background:${C.borde};margin:24px 0}
...
function StatusBadge({ status }) {
  const map = {
    ...
    pendiente: { label:"Pendiente", color:C.warning },
    cancelado: { label:"Cancelado", color:C.danger },
    approved:  { label:"Aprobado",  color:C.success },
    pending:   { label:"Pendiente", color:C.warning },
    failed:    { label:"Fallido",   color:C.danger },
  };
  ...
  return (
    <div className="adm-chart-wrap">
      ...
      <Bar
        data={{
          labels,
          datasets:[{ data:values, backgroundColor:"rgba(201,168,76,0.2)", borderColor:C.oro, borderWidth:1.5, borderRadius:2, borderSkipped:false, hoverBackgroundColor:"rgba(232,196,106,0.4)" }],
        }}
        options={{
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", zIndex:1100, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px 16px" }}>
      <div onClick={(e)=>e.stopPropagation()} style={{ background:C.verde, border:`0.5px solid rgba(201,168,76,0.35)`, width:"100%", maxWidth:420, padding:"32px" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={confirm} disabled={loading} className="adm-btn"
            style={{ flex:2, borderColor:"rgba(46,204,113,0.5)", color:C.success, background:"rgba(46,204,113,0.08)" }}>
      ...
      <div onClick={(e)=>e.stopPropagation()} style={{ background:C.verde, border:`0.5px solid rgba(201,168,76,0.3)`, width:"100%", maxWidth:540, maxHeight:"90vh", overflowY:"auto" }}>
        ...
        <div style={{ padding:"22px 28px", display:"flex", flexDirection:"column", gap:16 }}>
          {[
            ...
            { title:"Estado del pedido", rows:[["Payment ID",order.payment_id||"—"]] },
          ].map(({ title, rows }) => (
  ...
  return (
    <>
      ...
      <div className="adm-sec-header">
        <div>
          <span className="adm-sec-title">Pedidos</span>
          <span className="adm-sec-count">({filteredOrders.length})</span>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Total</th><th>Estado</th><th>Región</th><th></th>
            </tr>
          </thead>
          <tbody>
            {orders.length === 0 ? (
              ...
              <tr key={o.id} className="clickable" onClick={() => setSelOrder(o)}>
                <td style={{ fontSize:11, color:"rgba(245,240,232,0.45)", whiteSpace:"nowrap" }}>{formatDate(o.created_at)}</td>
                <td>
...
function VistaClientes({ clientes, tipo }) {
  const label = tipo === "registrados" ? "Clientes registrados" : "Suscriptores newsletter";
  const sublabel = tipo === "registrados"
    ? "Usuarios con cuenta creada en el sitio"
    : "Personas suscritas al newsletter con código ESSENZA15";

  const headers = tipo === "registrados"
  ...
  return (
    <>
      <div className="adm-kpi-grid" style={{ gridTemplateColumns:"repeat(2,1fr)" }}>
        <div className="adm-kpi">
          <div className="adm-kpi-label">Total {label.toLowerCase()}</div>
          <div className="adm-kpi-value">{clientes.length}</div>
          <div className="adm-kpi-sub">{sublabel}</div>
        </div>
      ...
      <div className="adm-table-wrap">
        <table>
          ...
          <tbody>
            ...
            ) : tipo === "registrados" ? clientes.map((c,i) => (
              <tr key={i}>
...
function VistaMarketing({ tipo }) {
  const configs = {
    meta_conv:    { title:"Meta Ads · con conversión",    desc:"Visitantes que llegaron desde Meta Ads y completaron una compra.", tabla:["Nombre","Email","Producto","Fecha","Valor"] },
    meta_noconv:  { title:"Meta Ads · sin conversión",    desc:"Visitantes que llegaron desde Meta Ads pero no compraron. Candidatos para retargeting.", tabla:["Email","Fuente","Fecha","Página visitada"] },
    whatsapp:     { title:"WhatsApp · con conversión",    desc:"Clientes que contactaron por WhatsApp y convirtieron en compra.", tabla:["Nombre","Teléfono","Email","Producto","Fecha"] },
    retargeting:  { title:"Retargeting",                  desc:"Visitantes sin conversión listos para campañas de reimpacto. Se construye automáticamente desde Meta Ads sin conversión.", tabla:["Email","Visitas","Último contacto","Canal"] },
    pixel:        { title:"Pixel Meta Ads",               desc:null },
  };
    ...
    return (
      <>
        <div className="adm-info-box">
          El <strong>Pixel de Meta Ads</strong> permite rastrear conversiones, optimizar campañas y construir audiencias personalizadas. Para activarlo necesitas agregar tu Pixel ID en el archivo <strong>public/index.html</strong> o en el componente raíz de la app.
        </div>
        <div className="adm-kpi-grid" style={{ gridTemplateColumns:"1fr" }}>
          <div className="adm-kpi">
            ...
            <div className="adm-kpi-sub">Para activarlo, dile a Claude Code: "Instala el pixel de Meta Ads con ID XXXXXXX"</div>
          </div>
        </div>
        <div className="adm-info-box" style={{ marginTop:20 }}>
          <strong>Eventos a configurar:</strong><br/>
          PageView · ViewContent (producto) · AddToCart · InitiateCheckout · Purchase
        </div>
      </>
    );
  }
  ...
  return (
    <>
      <div className="adm-info-box">
        {cfg.desc} <strong>Esta sección requiere crear las tablas en Supabase.</strong> Las tablas se crean automáticamente cuando ejecutes el SQL de configuración.
      </div>
      <div className="adm-kpi-grid" style={{ gridTemplateColumns:"repeat(2,1fr)" }}>
        <div className="adm-kpi">
      ...
      <div className="adm-table-wrap">
        <table>
          <thead><tr>{cfg.tabla.map(h=><th key={h}>{h}</th>)}</tr></thead>
          <tbody>
            <tr><td colSpan={cfg.tabla.length} className="td-empty">
              Ejecuta el SQL de configuración para activar esta sección
            </td></tr>
          </tbody>
        </table>
      </div>
...
const NAV = [
  { section:"Ventas" },
  { key:"today",       label:"Ventas hoy",            icon:"ti-chart-bar",        grupo:"ventas" },
  { key:"week",        label:"Ventas semana",          icon:"ti-calendar-week",    grupo:"ventas" },
  { key:"month",       label:"Ventas mes",             icon:"ti-calendar-month",   grupo:"ventas" },
  { divider:true },
  { section:"Clientes" },
  { key:"registrados", label:"Registrados",            icon:"ti-users",            grupo:"clientes" },
  { key:"newsletter",  label:"Newsletter",             icon:"ti-mail",             grupo:"clientes" },
  { divider:true },
  { section:"Marketing" },
  { key:"meta_conv",   label:"Meta Ads · conv.",       icon:"ti-brand-meta",       grupo:"marketing" },
  { key:"whatsapp",    label:"WhatsApp · conv.",       icon:"ti-brand-whatsapp",   grupo:"marketing" },
  { key:"meta_noconv", label:"Meta Ads · sin conv.",   icon:"ti-target",           grupo:"marketing" },
  { key:"retargeting", label:"Retargeting",            icon:"ti-refresh",          grupo:"marketing" },
  { divider:true },
  { section:"Configuración" },
  { key:"pixel",       label:"Pixel Meta Ads",         icon:"ti-brand-meta",       grupo:"config" },
  { key:"pedidos",     label:"Todos los pedidos",      icon:"ti-list",             grupo:"pedidos" },
];
...
export default function AdminDashboard() {
  ...
  /* badges dinámicos */
  const badges = {
    today: salesToday > 0 ? formatCLP(salesToday).replace("$","$") : "0",
    week:  formatCLP(salesWeek).replace(/\$(\d+\.\d+)/,"$$$1"),
    month: formatCLP(salesMonth),
    registrados: customerCount || 0,
    newsletter:  newsletterCount || 0,
  };

  const navItems = NAV.filter(n => n.key || n.section || n.divider);
    ...
    <div className="adm-layout">
      ...
      <aside className="adm-sidebar">
        ...
        {navItems.map((item, i) => {
          ...
          const badgeVal = badges[item.key];
      ...
      <main className="adm-main">
        <div className="adm-topbar">
          <div className="adm-topbar-title">
            {TITLES[vista]?.split("·")[0]}
            {TITLES[vista]?.includes("·") && <em>· {TITLES[vista].split("·")[1]}</em>}
          </div>
